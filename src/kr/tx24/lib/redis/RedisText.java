...
/**
 * Redis String Connection 관리 (Codec 없이 순수 String 처리)
 * 
 * <p><b> Redis 설계 방안</b></p>
 * <ul>
...
public final class RedisText {
    ...
    private static synchronized void initClient() {
        ...
        if (isShuttingDown.get()) {
            //logger.debug("RedisString is shutting down, skipping initialization");
            return;
        }
        ...
        try {
            ...
            // Redis Client 생성 (StringCodec 사용)
            ...
            logger.info("RedisString  : initialized ");
            ...
            logger.error("RedisString client initialization failed: {}", SystemUtils.getRedisSystemUri(), e);
        }
    }
    ...
    public static StatefulRedisConnection<String, String> getConnection() {
        // shutdown 중이면 예외 발생
        if (isShuttingDown.get()) {
            throw new IllegalStateException("RedisString is shutting down");
        }
     ...
     * @throws RuntimeException 재연결 실패 시
     */
    private static synchronized StatefulRedisConnection<String, String> reconnect() {
        // shutdown 중이면 재연결하지 않음
        if (isShuttingDown.get()) {
            throw new IllegalStateException("RedisString is shutting down, cannot reconnect");
        }
        ...
        try {
            logger.info("RedisString connection is not available. Attempting to reconnect...");
            ...
            logger.warn("RedisString reconnection successful");
            return connection;
            
        } catch (Exception e) {
            logger.warn("RedisString reconnection failed", e);
            throw new RuntimeException(
                "RedisString connection is not available and reconnection failed. " +
     ...
     * <p><b>사용 예:</b></p>
     * <pre>
     * RedisString.sync().set("key", "value");
     * String result = RedisString.sync().get("key");
     * </pre>
     * 
     ...
     * <p><b>사용 예:</b></p>
     * <pre>
     * RedisFuture<String> future = RedisString.async().set("key", "value");
    ...
    public static synchronized void shutdown() {
        ...
        if (!isShuttingDown.compareAndSet(false, true)) {
            logger.debug("RedisString shutdown already in progress");
            return;
        }
        
        // 초기화되지 않았으면 바로 리턴 (중요!)
        if (!isInitialized.get()) {
        	//logger.debug("RedisString was not initialized, skipping shutdown");
            return;
        }
        ...
        if (client != null) {
            try {
                ...
                logger.warn("Error shutting down RedisString client: {}", e.getMessage());
            } finally {
                client = null;
            }
        }
        ...
        logger.info("RedisString shutdown");
    }
    ...
    public static String getInfo() {
        if (!isConnected()) {
            return "RedisString: Not connected";
        }
        ...
        try {
            ...
            return "RedisString: Error getting info - " + e.getMessage();
        }
    }
